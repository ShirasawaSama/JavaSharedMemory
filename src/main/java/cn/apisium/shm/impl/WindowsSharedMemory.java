...
public class WindowsSharedMemory implements SharedMemory {
    ...
    private static final int SECTION_QUERY = 0x0001, SECTION_MAP_WRITE = 0x0002, SECTION_MAP_READ = 0x0004,
            ...
            SECTION_ALL_ACCESS = SECTION_QUERY | SECTION_MAP_WRITE | SECTION_MAP_READ | SECTION_MAP_EXECUTE
                    | SECTION_EXTEND_SIZE | SECTION_MAP_EXECUTE_EXPLICIT;
    ...
    static {
        if (CABI.SYSTEM_TYPE == CABI.SystemType.Windows) {
            ...
            createFileMapping = linker.downcallHandle(kernel.lookup("CreateFileMappingA").orElseThrow(),
                    FunctionDescriptor.of(
                            ...
                            ValueLayout.JAVA_INT,
                            ValueLayout.ADDRESS));
            openFileMapping = linker.downcallHandle(kernel.lookup("OpenFileMappingA").orElseThrow(),
                    FunctionDescriptor.of(
                            ValueLayout.ADDRESS,
                            ValueLayout.JAVA_INT,
                            ValueLayout.JAVA_INT,
            ...
            mapViewOfFile = linker.downcallHandle(kernel.lookup("MapViewOfFile").orElseThrow(), FunctionDescriptor.of(
                    ...
                    ValueLayout.JAVA_INT,
                    ValueLayout.JAVA_INT));
            unmapViewOfFile = linker.downcallHandle(kernel.lookup("UnmapViewOfFile").orElseThrow(),
    ...
    public WindowsSharedMemory(String name, long size) throws Throwable {
        this(name, size, true);
    }

    public WindowsSharedMemory(String name, long size, boolean isCreate) throws Throwable {
        if (CABI.SYSTEM_TYPE != CABI.SystemType.Windows)
        ...
        if (hMapFile.toRawLongValue() == 0)
            throw new IllegalStateException("CreateFileMapping failed.");
        try {
            pBuf = (MemoryAddress) mapViewOfFile.invokeExact((Addressable) hMapFile,
                    SECTION_MAP_WRITE | SECTION_MAP_READ, 0, 0, size);
            if (pBuf.toRawLongValue() == 0)
                throw new IllegalStateException("MapViewOfFile failed.");
        } catch (Throwable th) {
